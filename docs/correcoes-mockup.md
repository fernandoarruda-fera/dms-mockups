...
## Passo 4 — tela77 ActionIA + tela78 Alertas + tela79 Match Engine (2026-05-31)

Origem: 3 últimas telas pendentes do mapa (concepts ACTIONIA, ALERT, MATCH-ENGINE) — fecham 100% de cobertura mockup desses concepts. Arquivos placeholder pré-existentes substituídos por implementação concept-aligned.

- **tela77 (Agentes ActionIA — biblioteca)** — KPIs (6 total / 4 ativos / 3 em uso / 2 substituem stage humano / 2 rascunhos). Filtros (busca, tipo, status, "stage humano?"). Tabela lista 6 agentes mock cobrindo os 4 tipos do concept (`actionai`/`validacao`/`roteamento`/`notificacao`): Validador NF Fiscal, Classificador Email MAILIA, Roteador Aprovação por Alçada, Notificador Slack #compras, Extrator PDF Boleto, Validador Match Engine (legado). Drawer 4 abas (Detalhes / Config JSON / Em uso por workflows / Histórico) destacando `can_replace_stage_user` (badge violeta com aviso "exige Admin Geral"). Modal Novo Agente: seletor visual de tipo (4 cards), nome/descrição/objetivo/resultado, skills com datalist, config form dinâmico (mostrado para ActionAI: modelo + max_tokens + prompt_template), checkbox `can_replace_stage_user` com explicação. Aviso sobre ponte ActionIA↔Stage (`workflow_stages.actionia_id` + `escalation_role_id`).
- **tela78 (Alertas Configuráveis)** — KPIs (5 ativos / 2 críticas / 38 disparos 24h / 14 throttled / 1 sem destinatários ⚠️). 2 abas top-level: **📐 Regras (6)** e **📜 Histórico de disparos (12)**. Aba Regras: filtros (busca, severidade, canal, status), tabela com nome / severidade (info/atenção/crítico) / canais (chips inapp/email/slack/push) / trigger DSL / disparos / cooldown / status. 6 regras mock (SLA crítico, NF sem PO, Pedido excedeu alçada, Cotação +5d, Contrato vence 30d, Match ambiguidade rascunho). Aba Histórico: tabela 12 disparos com timestamp + regra + contexto + canal + destinatário + status entrega (entregue/throttled/falha SMTP). Drawer Nova Regra: nome + severidade radio (3 opções com badge) + 4 cards de canal multi-select + textarea DSL com syntax highlight (escuro tipo terminal) + mensagem template com preview + destinatarios_expr + throttling cooldown. Aviso "crítico exige ≥2 canais".
- **tela79 (Regras de Match Engine)** — KPIs (5 ativas / 142 match único 24h / 9 órfãos / 2 ambíguos ⚠️ / confidence 0.91 médio). Filtros (busca, modo, input_type, status). Tabela com workflow alvo / modo (manual/ia_assistida/ia_validacao com chip e ícone) / input type (email/documento/evento_api) / tags (AND chips) / confidence min / % match único com barra colorida / status. 6 regras mock. Drawer Nova Regra: workflow alvo dropdown + input_type + 3 cards de modo com explicação ("Manual: sem IA / IA Assistida: propõe + edita / IA + Validação: simula contra base, bloqueia se ambíguo"), editor de mapping (3 linhas grid input→workflow + obrig sim/não), tags AND, confidence slider (0.5–1.0, default 0.85), painel **Simulação contra base** (112 únicos / 10 órfãos / 2 ambíguos) com aviso "modo ia_validacao bloquearia save". Aviso destacado em vermelho: "unicidade obrigatória — >1 match = bloqueio + alerta crítico, NUNCA heurística".
